...
실행 중인 workspace의 종료 예정 시각까지 **5시간 미만**으로 남으면 Slack으로 한 번 알립니다.
정확히 5시간이면 알리지 않으며, 첫 확인부터 5시간 미만이어도 알립니다.
5시간 이상으로 연장되거나 중지 후 재시작하면 다시 알릴 수 있습니다.
종료 예정 시각이 없거나 해석할 수 없으면 남은 시간 알림은 건너뜁니다.
알림에는 감지 당시 남은 시간과 종료 예정 시각을 표시하며, 시간 확인은 아래의 정기 실행 주기를 따릅니다.
...
- 상세 조회가 HTTP 404를 반환하거나 전체 목록에 `소유자/이름`이 없으면 해당 대상만 건너뛰고 나머지는 계속 처리합니다. 전부 없더라도 정상 완료합니다. 등록은 유지하여 다음 실행에서 다시 조회합니다.
- 건너뛴 대상은 중지 알림이나 자동 시작을 발생시키지 않습니다. 이전 미전송 알림은 기존 재시도 정책을 따르며, 다시 조회되는 대상은 새 기준 상태부터 감시합니다.
- 인증·권한 오류(401/403), 서버 오류, 불완전한 목록, 중복 이름 등은 기존처럼 실행을 실패시킵니다.
- `소유자/이름`이 여러 workspace와 일치하면 해당 항목 번호와 함께 오류를 표시합니다. 중복되는 경우 숫자 ID를 사용하세요.
...
존재하지 않는 대상은 건너뛰며, 인증·권한 오류가 발생하면 해당 실행이 실패합니다. 승인 전에 대상을 확인하고,
