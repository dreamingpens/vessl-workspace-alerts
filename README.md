...
- 감시 대상 하나라도 조회에 실패하면 그 실행의 상태는 갱신하지 않습니다. 삭제한 ID는 Secret에서 제거하세요.
...
- 이전 상태와 미전송 알림은 `.monitor/state.enc`에 Fernet으로 암호화해 커밋합니다.
  이름·ID·상태를 공개 로그에 출력하지 않습니다. 암호문 갱신 시각과 workflow 성공/실패는 공개됩니다.
...
`VESSL_WORKSPACE_IDS` Secret에 추가하면 됩니다. 알림에는 workspace 이름과 소유자가 표시됩니다.
...
다른 팀은 이 저장소를 공개 저장소로 복사한 뒤 자체 Secrets를 등록해 사용할 수 있습니다.
